 ...
 * 修改备注：
 */
public class ProblemPictureAdapter extends BaseAdapter {
...
//    private ImageCache manager;
    ...
    public ProblemPictureAdapter(Context context, List<PictureItemBean> pictureItemBeans) {
        mContext = context;
...
//        manager = ImageCache.getInstanse(context);
    ...
    public View getView(final int pos, View view, ViewGroup arg2) {
        ...
        final ViewHolder vh;
        if (view == null) {
            view = LayoutInflater.from(mContext).inflate(R.layout.problem_picture_item, null);
            vh = new ViewHolder();
            ...
            vh.iv_add = (ImageView) view.findViewById(R.id.iv_add);
        ...
        if(pictureItemBeans.size()==1){
            vh.iv_add.setImageResource(R.mipmap.opinion_photo);
        }else {
            vh.iv_add.setImageResource(R.mipmap.opinion_photo_add);
        }
        ...
        String pictureurl = pictureItemBeans.get(pos).getPictureUrl();
            if (!TextUtils.isEmpty(pictureurl)) {
                imageLoader.displayImage(Constants.BaseImageUrl + pictureurl, vh.iv_picture, PictureOption.getSimpleRoundOptions());
            }
            else {
