...
//    private ImageCache manager;
    ...
    public AddPictureAdapter(Context context, List<PictureItemBean> pictureItemBeans) {
        mContext = context;
...
//        manager = ImageCache.getInstanse(context);
    ...
    public View getView(final int pos, View view, ViewGroup arg2) {
        ...
        final ViewHolder vh;
        if (view == null) {
            view = LayoutInflater.from(mContext).inflate(R.layout.add_picture_item, null);
            vh = new ViewHolder();
        ...
        String pictureurl = pictureItemBeans.get(pos).getPictureUrl();
            if (!TextUtils.isEmpty(pictureurl)) {
                imageLoader.displayImage(Constants.BaseImageUrl + pictureItemBeans.get(pos).getPictureUrl(), vh.iv_picture, PictureOption.getSimpleOptions());
...
//                manager.loadImages(vh.iv_picture, Constants.BaseImageUrl+pictureurl, false);
                ...
                if(!TextUtils.isEmpty(picturePath)){
                    Bitmap bitmap = BitmapUtils.getSmallBitmap(pictureItemBeans.get(pos).getPicturePath());
